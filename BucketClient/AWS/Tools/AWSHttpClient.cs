...
namespace BucketClient.AWS
{
    internal class AWSHttpClient
    {
        ...
        public async Task<OperationResult> SendRequest(HttpMethod method, string endpoint, byte[] content, HttpStatusCode successDef = HttpStatusCode.OK)
        {
            return await SendRequest(method, new Uri(endpoint), content, successDef);
        }

        public async Task<OperationResult> SendRequest(HttpMethod method, Uri endpoint, byte[] content, HttpStatusCode successDef = HttpStatusCode.OK)
        {
            
            
            var c = content != null ? new ByteArrayContent(content): null;
            string mime = content != null ? content.GetFileType().Mime: "text/plain";
            return await SendRequest(method, endpoint, c, mime, successDef);
        }

        

        public async Task<OperationResult> SendRequest(HttpMethod method, Uri endpoint, HttpContent content, string type = "text/plain", HttpStatusCode successDef = HttpStatusCode.OK)
        {
        ...
        public async Task<OperationResult> SendRequest(HttpMethod method, string endpoint, string content = null, string type = "text/plain", HttpStatusCode successDef = HttpStatusCode.OK)
        {
            return await SendRequest(method, new Uri(endpoint), content, type, successDef);

        }
        public async Task<OperationResult> SendRequest(HttpMethod method, Uri endpoint, string content = null, string type = "text/plain", HttpStatusCode successDef = HttpStatusCode.OK)
        {
